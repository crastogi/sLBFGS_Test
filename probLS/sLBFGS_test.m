...
function [path, function_values, grad_norm] = sLBFGS_test(ff, x0, ...
    ...
    global xt_var wk_var tot_var diff_var;
    ...
    for k = 0:maxEpoch
        ...
        for t = 1:m
            ...
            [f_xt,grad_f_xt,fvar_xt,dfvar_xt,~,~, gxt, txt] = ff(x_t', sidx);
            [f_wk,grad_f_wk,fvar_wk,dfvar_wk,~,~, gwk, twk] = ff(w_k', sidx);
            f_t = (f_xt + fFull) - f_wk;
            ...
            xt_var =  [xt_var; [fvar_xt, dot(effGrad.^2, dfvar_xt)]];
            wk_var =  [wk_var; [fvar_wk, dot(effGrad.^2, dfvar_wk)]];
            tot_var = [tot_var; [fVar_f*var_scale dot(effGrad.^2, fVar_df)]];
            
            diff_f = 1/(batchsize-1)*(sum((txt-twk).^4/4)/batchsize - (sum((txt-twk).^2/2)/batchsize)^2);
            diff_df = 1/(batchsize-1)*(sum((gxt-gwk).^2,1)/batchsize - (sum((gxt-gwk),1)/batchsize).^2);
            diff_var = [diff_var [diff_f dot(effGrad.^2, diff_df)]];
            
            disp(log10([diff_f fVar_f*var_scale norm(diff_df) norm(fVar_df)]));
            ...
            if useLineSearch
                ...
                if (r < 1)
                    ls_out = ls_function(ff, x_t', f_t, v_t', -effGrad',...
                        eta/norm(v_t), verbosity, [], [], abs(diff_f), ...
                        abs(diff_df'), 1, [], fFull, mu_k', w_k');
                else
                    ls_out = ls_function(ff, x_t', f_t, v_t', -effGrad',...
                        eta, verbosity, [], [], abs(diff_f), ...
                        abs(diff_df'), 1, [], fFull, mu_k', w_k');
                end
