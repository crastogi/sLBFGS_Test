...
public class MultiModeModel extends Model{
	...
	private int l, nThreads, totFeatures, flankLength;
	...
	private long[] R1FProbes;
	public double[] betas, modeAffinities;
	private double[] gradients, R1R0Prob, R1R0FProb, activeFeatures;
	...
	public CompactGradientOutput stochasticEvaluate() throws Exception {
		...
		gradients			= Array.scalarMultiply(gradients, currBatchSize/(Z_FFM*R0Model.getZ()));
	...
	private double[] reverseGradients(double[] input) {
		double[] output		= new double[totFeatures];

		for (int i=0; i<totFeatures; i++) {
			output[i] = input[(int) reverseMap[i]];
		}
		return output;
	}
	...
	public CompactGradientOutput StochasticGradientEvaluator() {
		...
		if (type==0) {
			for (int i=0; i<currBatchSize; i++) {
				for (int j=0; j<nModes; j++) {
					modes.get(j).swNucleotide(R1FProbes[currBatchIdx[i]], kappas);
				}
				sum = nsBindingValue;
				for (int j=0; j<totalFrames; j++) {
					sum += kappas[j];
			...
			for (int i=0; i<currBatchSize; i++) {
				for (int j=0; j<nModes; j++) {
					modes.get(j).swNucleotideDinucleotide(R1FProbes[currBatchIdx[i]], kappas);
				}
				sum = nsBindingValue;
				for (int j=0; j<totalFrames; j++) {
					sum += kappas[j];
			...
			for (int i=0; i<currBatchSize; i++) {
				for (int j=0; j<nModes; j++) {
					modes.get(j).swNucleotideShape(R1FProbes[currBatchIdx[i]], kappas);
				}
				sum = nsBindingValue;
				for (int j=0; j<totalFrames; j++) {
					sum += kappas[j];
			...
			for (int i=0; i<currBatchSize; i++) {
				for (int j=0; j<nModes; j++) {
					modes.get(j).swNucleotideDinucleotideShape(R1FProbes[currBatchIdx[i]], kappas);
				}
				sum = nsBindingValue;
				for (int j=0; j<totalFrames; j++) {
					sum += kappas[j];
			...
			for (int i=0; i<currBatchSize; i++) {
				for (int j=0; j<nModes; j++) {
					modes.get(j).swNucleotideNoFlank(R1FProbes[currBatchIdx[i]], kappas);
				}
				sum = nsBindingValue;
				for (int j=0; j<totalFrames; j++) {
					sum += kappas[j];
			...
			for (int i=0; i<currBatchSize; i++) {
				for (int j=0; j<nModes; j++) {
					modes.get(j).swNucleotideDinucleotideNoFlank(R1FProbes[currBatchIdx[i]], kappas);
				}
				sum = nsBindingValue;
				for (int j=0; j<totalFrames; j++) {
					sum += kappas[j];
			...
			for (int i=0; i<currBatchSize; i++) {
				for (int j=0; j<nModes; j++) {
					modes.get(j).swNucleotideShapeNoFlank(R1FProbes[currBatchIdx[i]], kappas);
				}
				sum = nsBindingValue;
				for (int j=0; j<totalFrames; j++) {
					sum += kappas[j];
			...
			for (int i=0; i<currBatchSize; i++) {
				for (int j=0; j<nModes; j++) {
					modes.get(j).swNucleotideDinucleotideShapeNoFlank(R1FProbes[currBatchIdx[i]], kappas);
				}
				sum = nsBindingValue;
				for (int j=0; j<totalFrames; j++) {
					sum += kappas[j];
