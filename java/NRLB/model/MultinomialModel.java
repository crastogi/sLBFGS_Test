...
public class MultinomialModel extends Model{
	...
	private int l, k, nThreads, totFeatures, flankLength, nShapeClasses;
	...
	private long[] R1FProbes; // unwrapped
	private double[] R1R0Prob;
	private double[] R1R0FProb; //unwrapped
	private Data data;
	...
	// Need this to unwrap data
	private void prepareSGData() {
		int currIdx = 0;
		R1FProbes = new long[nCount];
		R1R0FProb = new double[nCount];
		
		// loop over and unwrap
		for (int i=0; i<R1Probes.length; i++) {
			for (int rep=0; rep<R1Counts[i]; rep++) {
				R1FProbes[currIdx] = R1Probes[i];
				R1R0FProb[currIdx] = R1R0Prob[i];
				currIdx++;
			}
		}
	}
	...
	public CompactGradientOutput stochasticEvaluate() throws Exception {
		...
		double[] nucGradients, dinucGradients = null, shapeGradients = null;
		CompactGradientOutput output;
		
		resetGradients();		// Clear old gradient values
		Z.recursiveGradient();	//Calculate gradient of Z
		Z_FFM				= Z.getZ()/R0Model.getZ() + nsBindingValue;
		functionValue		= currBatchSize*Math.log(Z_FFM);
		nucGradients		= Array.scalarMultiply(Z.getNucGradients(), currBatchSize/(Z_FFM*R0Model.getZ()));
		if (isDinuc)		dinucGradients	= Array.scalarMultiply(Z.getDinucGradients(), currBatchSize/(Z_FFM*R0Model.getZ()));
		if (isShape)		shapeGradients	= Array.scalarMultiply(Z.getShapeGradients(), currBatchSize/(Z_FFM*R0Model.getZ()));

		//Need the proper orientation for sliding windows
		reverseBetas();			
		reverseGradients();
		output = StochasticGradientEvaluator();
		// Update values
		functionValue		-= output.functionValue;
		nsBindingGradient	-= getNSGradient(output);
		nucGradients		= Array.subtract(nucGradients, getNucVector(output));
		if (isDinuc)		dinucGradients	= Array.subtract(dinucGradients, getDinucVector(output));
		if (isShape)		shapeGradients	= Array.subtract(shapeGradients, getShapeVector(output));
		//Return to the original state
		reverseBetas();			
		reverseGradients();
		
		// Compute final values
		nsBindingGradient	= nsBindingValue*nsBindingGradient/currBatchSize + 2*lambda*getNSBeta();
		
		functionValue		= functionValue/currBatchSize + lambda*(((double) currBatchSize)/((double) nCount))*Math.pow(Array.norm(getPositionVector()),2);
		nucGradients		= Array.addScalarMultiply(Array.scalarMultiply(nucGradients, 1.0/((double) currBatchSize)), 2*lambda*(1.0/((double) nCount)), nucBetas);
		if (isDinuc)	dinucGradients = Array.addScalarMultiply(Array.scalarMultiply(dinucGradients, 1.0/((double) currBatchSize)), 2*lambda*(1.0/((double) nCount)), dinucBetas);
		if (isShape)	shapeGradients = Array.addScalarMultiply(Array.scalarMultiply(shapeGradients, 1.0/((double) currBatchSize)), 2*lambda*(1.0/((double) nCount)), shapeBetas);
...
//					output += R1R0Prob.get(i)*swNucleotideDinucleotideShapeNoFlank(R1Probes.get(i));
	...
	public CompactGradientOutput StochasticGradientEvaluator() {
		double functionValue = 0, nsGradient = 0, probeSum;
		double[] nucGradients	= new double[4*k];
		double[] dinucGradients	= null;
		double[] shapeGradients	= null;
		...
		if (type==0) {
			for (int i=0; i<currBatchSize; i++) {
				probeSum		= swGradNucleotide(R1FProbes[currBatchIdx[i]], 1, nucGradients);
				nsGradient		+= (1/probeSum-1/Z_FFM);
				functionValue	+= Math.log(R1R0FProb[currBatchIdx[i]]*probeSum);
			}
		} else if(type==1) {
			for (int i=0; i<currBatchSize; i++) {
				probeSum		= swGradNucleotideDinucleotide(R1FProbes[currBatchIdx[i]], 1, nucGradients, dinucGradients);
				nsGradient		+= (1/probeSum-1/Z_FFM);
				functionValue	+= Math.log(R1R0FProb[currBatchIdx[i]]*probeSum);
			}
		} else if(type==2) {
			for (int i=0; i<currBatchSize; i++) {
				probeSum		= swGradNucleotideShape(R1FProbes[currBatchIdx[i]], 1, nucGradients, shapeGradients);
				nsGradient		+= (1/probeSum-1/Z_FFM);
				functionValue	+= Math.log(R1R0FProb[currBatchIdx[i]]*probeSum);
			}
		} else if(type==3){
			for (int i=0; i<currBatchSize; i++) {
				probeSum		= swGradNucleotideDinucleotideShape(R1FProbes[currBatchIdx[i]], 1, nucGradients, dinucGradients, shapeGradients);
				nsGradient		+= (1/probeSum-1/Z_FFM);
				functionValue	+= Math.log(R1R0FProb[currBatchIdx[i]]*probeSum);
			}
		} else if(type==4){
			for (int i=0; i<currBatchSize; i++) {
				probeSum		= swGradNucleotideNoFlank(R1FProbes[currBatchIdx[i]], 1, nucGradients);
				nsGradient		+= (1/probeSum-1/Z_FFM);
				functionValue	+= Math.log(R1R0FProb[currBatchIdx[i]]*probeSum);
			}
		} else if(type==5) {
			for (int i=0; i<currBatchSize; i++) {
				probeSum		= swGradNucleotideDinucleotideNoFlank(R1FProbes[currBatchIdx[i]], 1, nucGradients, dinucGradients);
				nsGradient		+= (1/probeSum-1/Z_FFM);
				functionValue	+= Math.log(R1R0FProb[currBatchIdx[i]]*probeSum);
			}
		} else if(type==6) {
			for (int i=0; i<currBatchSize; i++) {
				probeSum		= swGradNucleotideShapeNoFlank(R1FProbes[currBatchIdx[i]], 1, nucGradients, shapeGradients);
				nsGradient		+= (1/probeSum-1/Z_FFM);
				functionValue	+= Math.log(R1R0FProb[currBatchIdx[i]]*probeSum);
			}
		} else if(type==7) {
			for (int i=0; i<currBatchSize; i++) {
				probeSum		= swGradNucleotideDinucleotideShapeNoFlank(R1FProbes[currBatchIdx[i]], 1, nucGradients, dinucGradients, shapeGradients);
				nsGradient		+= (1/probeSum-1/Z_FFM);
				functionValue	+= Math.log(R1R0FProb[currBatchIdx[i]]*probeSum);
			}
		}
