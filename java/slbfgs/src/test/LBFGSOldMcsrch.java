...
public class LBFGSOldMcsrch
{
	...
	public static void mcsrch ( int n , double[] x , double f , double[] g , double[] s , int is0 , double[] stp , double ftol , double xtol , int maxfev , int[] info , int[] nfev , double[] wa )
	{
		...
		{
			infoc[0] = 1;
			if ( n <= 0 || stp[0] <= 0 || ftol < 0 || LBFGSOldMinimizer.gtol < 0 || xtol < 0 || LBFGSOldMinimizer.stpmin < 0 || LBFGSOldMinimizer.stpmax < LBFGSOldMinimizer.stpmin || maxfev <= 0 ) 
				return;
			...
			finit = f;
			dgtest = ftol*dginit;
			width = LBFGSOldMinimizer.stpmax - LBFGSOldMinimizer.stpmin;
			width1 = width/p5;
		...
		while ( true )
		{
			if ( info[0] != -1 )
			{
				...
				stp[0] = Math.max ( stp[0] , LBFGSOldMinimizer.stpmin );
				stp[0] = Math.min ( stp[0] , LBFGSOldMinimizer.stpmax );
			...
			if ( stp[0] == LBFGSOldMinimizer.stpmax && f <= ftest1 && dg <= dgtest ) info[0] = 5;

			if ( stp[0] == LBFGSOldMinimizer.stpmin && ( f > ftest1 || dg >= dgtest ) ) info[0] = 4;

			if ( nfev[0] >= maxfev ) info[0] = 3;

			if ( brackt[0] && stmax - stmin <= xtol * stmax ) info[0] = 2;

			if ( f <= ftest1 && Math.abs ( dg ) <= LBFGSOldMinimizer.gtol * ( - dginit ) ) info[0] = 1;
			...
			if ( stage1 && f <= ftest1 && dg >= Math.min ( ftol , LBFGSOldMinimizer.gtol ) * dginit ) stage1 = false;
